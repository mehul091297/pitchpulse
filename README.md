...
- [x] Player availability tracking wired in (`src/availability.py` — injury/doubt/suspension status straight from FPL's own official data, no news scraping; `recommend_squad()` now excludes injured/suspended/unavailable players by default, degrading gracefully if no availability snapshot has been ingested yet; verified against real live data in Colab — a genuinely injured player (Saliba) is confirmed excluded from the optimizer's recommended squad)
- [x] Chip-timing strategy wired in (`recommend_chip_strategy()` — recommends which upcoming gameweek to play each of the season's four chips (Wildcard, Free Hit, Triple Captain, Bench Boost) for the current recommended squad, using FPL's own real fixture difficulty rating and blank/double-gameweek detection from `src/fixtures.py`; respects the real GW19/20 chip-half boundary. Verified with synthetic double-gameweek/blank-gameweek/sustained-trough scenarios, including a caught-and-fixed edge case where a gameweek missing from the fixture feed entirely was initially indistinguishable from every team having a real blank that week — then verified end-to-end against real live data in Colab)
- [x] Deployed — live at https://pitchpulse-bs7uhkkyuchigcpnxupi29.streamlit.app/
- [x] Player availability, chip strategy, and the new crew tools verified against real live data in Colab
- [ ] Demo recorded
